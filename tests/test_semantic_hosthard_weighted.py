...
class SemanticHostHardWeightedTests(unittest.TestCase):
    def _build_losses(self, **overrides) -> PrototypeLosses:
        kwargs = dict(
            ...
            lambda_semantic_hosthard_weighted=1.7,
            semantic_hosthard_margin_ref=0.1,
            semantic_hosthard_tau=0.2,
            semantic_hosthard_eps=1e-8,
    ...
    def test_hosthard_weighted_loss_matches_manual_computation(self):
        ...
        manual = losses._semantic_hosthard_weighted_loss(
            semantic_info=semantic_info,
            host_pairwise_logits=host_scores,
        )

        torch.testing.assert_close(outputs['loss_semantic_hosthard_weighted'], manual['loss'])
        torch.testing.assert_close(outputs['loss_semantic_hosthard_weighted_image'], manual['loss_image'])
        torch.testing.assert_close(outputs['loss_semantic_hosthard_weighted_text'], manual['loss_text'])
        torch.testing.assert_close(
            outputs['loss_semantic_hosthard_weighted_weighted'],
            outputs['lambda_semantic_hosthard_weighted'] * outputs['loss_semantic_hosthard_weighted'],
        )
    ...
    def test_batch_size_one_returns_zero(self):
        ...
        self.assertEqual(float(outputs['loss_semantic_hosthard_weighted'].item()), 0.0)
        self.assertEqual(float(outputs['loss_semantic_hosthard_weighted_image'].item()), 0.0)
        self.assertEqual(float(outputs['loss_semantic_hosthard_weighted_text'].item()), 0.0)

    def test_hosthard_scale_uses_dedicated_override(self):
        ...
        half_scaled = losses(
            ...
            semantic_text_teacher_embeddings=x,
            semantic_base_prototypes=base_prototypes,
            semantic_hosthard_weighted_loss_scale=0.5,
        )

        torch.testing.assert_close(
            half_scaled['loss_semantic_hosthard_weighted_weighted'],
            baseline['loss_semantic_hosthard_weighted_weighted'] * 0.5,
        )
        self.assertEqual(float(half_scaled['loss_semantic_hosthard_weighted_scale'].item()), 0.5)

    def test_hosthard_scale_defaults_to_semantic_pbt_scale(self):
        losses = self._build_losses()
        x = torch.randn(3, 4)
        base_prototypes = torch.randn(2, 4)
        ...
        self.assertEqual(float(outputs['loss_semantic_hosthard_weighted_scale'].item()), 0.25)

    def test_missing_semantic_structures_raise(self):
        losses = self._build_losses()
        x = torch.randn(3, 4)
        host_scores = torch.randn(3, 3)
        with self.assertRaisesRegex(ValueError, 'requires semantic_image_student_embeddings'):
            losses(
                x,
                x,
                x,
    ...
    def test_hosthard_weights_detach_host_scores(self):
        losses = self._build_losses()
        x = torch.randn(3, 4)
        base_prototypes = torch.randn(2, 4)
        host_scores = torch.randn(3, 3, requires_grad=True)
        ...
        outputs['loss_semantic_hosthard_weighted_weighted'].backward()
