...
def build_parser():
    ...
    parser.add_argument('--repro_enabled', type=_str2bool, nargs='?', const=True, default=False)
    parser.add_argument('--repro_seed', type=int, default=None)
    parser.add_argument('--repro_deterministic_algorithms', type=_str2bool, nargs='?', const=True, default=False)
    parser.add_argument('--repro_deterministic_warn_only', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_cudnn_benchmark', type=_str2bool, nargs='?', const=True, default=False)
    parser.add_argument('--repro_cudnn_deterministic', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_disable_tf32', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_dataloader_worker_seed', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_dataloader_generator', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_sampler_set_epoch', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_proto_deterministic_recompute', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_proto_eval_mode_recompute', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_proto_deterministic_transform', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_proto_sort_cache_by_id', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_proto_hash_logging', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_save_proto_runtime_state', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_stable_hardneg_tiebreak', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_hardneg_gap_logging', type=_str2bool, nargs='?', const=True, default=True)
    parser.add_argument('--repro_stable_eval_tiebreak', type=_str2bool, nargs='?', const=True, default=True)
...
def _finalize_args(args):
    ...
    args.repro_enabled = bool(getattr(args, 'repro_enabled', False))
    args.repro_seed = getattr(args, 'repro_seed', None)
    if args.repro_seed in ('', None):
        args.repro_seed = int(getattr(args, 'seed', 1))
