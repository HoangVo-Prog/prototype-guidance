...
class CLIP(nn.Module):
    ...
    def encode_image_intermediates(self, image, return_all: bool = False, average_attn_weights=True):
        if not isinstance(self.visual, VisionTransformer):
            projected_tokens = self.visual(image.type(self.dtype))
            return {
        ...
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
            [self.visual.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x],
            dim=1,
        )
        ...
        if return_all:
            ...
            outputs = self.visual.transformer([x], average_attn_weights=average_attn_weights)
            x = outputs[0]
            attention_weights = outputs[1]

        x = x.permute(1, 0, 2)  # LND -> NLD
        pre_projection_tokens = self.visual.ln_post(x)
        projected_tokens = pre_projection_tokens
        if self.visual.proj is not None:
            projected_tokens = pre_projection_tokens @ self.visual.proj
    ...
    def encode_text_intermediates(self, text, return_all: bool = False, average_attn_weights=True):
        ...
        if return_all:
            ...
            outputs = self.transformer([x], average_attn_weights=average_attn_weights)
            x = outputs[0]
            attention_weights = outputs[1]

        x = x.permute(1, 0, 2)  # LND -> NLD
        pre_projection_tokens = self.ln_final(x).type(self.dtype)
        projected_tokens = pre_projection_tokens @ self.text_projection
    ...
    def encode_image(self, image):
        outputs = self.encode_image_intermediates(image, return_all=False)
        return outputs['projected_tokens'], outputs['attention_weights']

    def encode_text(self, text):
        outputs = self.encode_text_intermediates(text, return_all=False)
        return outputs['projected_tokens'], outputs['attention_weights']

    def encode_text_all_atten(self, text, average_attn_weights=True):
        outputs = self.encode_text_intermediates(text, return_all=True, average_attn_weights=average_attn_weights)
        return outputs['projected_tokens'], outputs['attention_weights']
    
    def encode_image_all_atten(self, image, average_attn_weights=True):
        outputs = self.encode_image_intermediates(image, return_all=True, average_attn_weights=average_attn_weights)
        return outputs['projected_tokens'], outputs['attention_weights']

    def forward(self, image, text, return_all: bool = False, average_attn_weights=True):
        image_outputs = self.encode_image_intermediates(image, return_all=return_all, average_attn_weights=average_attn_weights)
        text_outputs = self.encode_text_intermediates(text, return_all=return_all, average_attn_weights=average_attn_weights)
        return (
